    ...
    hidden [void] _EditTags() {
        ...
        try {
            ...
            while (-not $done) {
                ...
                Add-Content -Path "/tmp/pmc-debug.log" -Value "[$(Get-Date -Format 'HH:mm:ss.fff')] [KanbanScreenV2] Rendering editor"
    ...
    hidden [void] _PickColor() {
        ...
        try {
            while (-not $done) {
                ...
                Add-Content -Path "/tmp/pmc-debug.log" -Value "[$(Get-Date -Format 'HH:mm:ss.fff')] [KanbanScreenV2] Rendering color picker"

...
if ($DebugLog -or $effectiveLogLevel -gt 0) {
    try {
        ...
        Write-Host "Debug logging enabled: $global:PmcTuiLogFile (Level $effectiveLogLevel)" -ForegroundColor Yellow
    }
    catch {
        # If log setup fails, disable logging and continue
        Write-Host "WARNING: Failed to setup logging: $_" -ForegroundColor Yellow

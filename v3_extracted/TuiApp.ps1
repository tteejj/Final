...
class TuiApp {
    ...
    hidden [KanbanBoard]$_kanbanBoard
    ...
    hidden [ThemeSelectorModal]$_themeSelector
    ...
    TuiApp([HybridRenderEngine]$engine, [FluxStore]$store) {
        ...
        $this._kanbanBoard = [KanbanBoard]::new()
        ...
        $this._themeSelector = [ThemeSelectorModal]::new($store)
    ...
    [void] Run() {
        ...
        while ($this._running) {
            ...
            try {
                ...
                $modalVisible = $this._projectInfoModal.IsVisible() -or $this._timeModal.IsVisible() -or $this._overviewModal.IsVisible() -or $this._globalNotesModal.IsVisible() -or $this._globalChecklistsModal.IsVisible() -or $this._commandPalette.IsVisible() -or $this._themeSelector.IsVisible()
                ...
                if ($needsRender) {
                    ...
                    if ($this._projectInfoModal.IsVisible()) { $this._projectInfoModal.Render($this._engine) }
                    if ($this._timeModal.IsVisible()) { $this._timeModal.Render($this._engine) }
                    if ($this._overviewModal.IsVisible()) { $this._overviewModal.Render($this._engine) }
                    if ($this._globalNotesModal.IsVisible()) { $this._globalNotesModal.Render($this._engine) }
                    if ($this._globalChecklistsModal.IsVisible()) { $this._globalChecklistsModal.Render($this._engine) }
                    if ($this._commandPalette.IsVisible()) { $this._commandPalette.Render($this._engine) }
                    if ($this._themeSelector.IsVisible()) { $this._themeSelector.Render($this._engine) }
    ...
    hidden [void] _HandleInput([ConsoleKeyInfo]$key) {
        ...
        if ($state.View.Editing) {
            ...
            if ($result -eq "Save") {
                ...
                if ($editInfo.RowId -eq "NEW") {
                    ...
                    $this._statusBar.ShowMessage("Item Added", "Success")
                } else {
                    # Update Existing
                    ...
                    $this._statusBar.ShowMessage("Item Updated", "Success")
                }
        ...
        if ($this._commandPalette.IsVisible()) {
            ...
            if ($result.Action -eq "Execute") {
                $cmd = $result.Command
                $this._statusBar.ShowMessage("Executing: $($cmd.Name)", "Info")

                # Execute the command based on its Key
                switch ($cmd.Key) {
                    ...
                    'L' { $this._timeModal.Open() }
                    'T' { $this._timeModal.Open() }
                    'O' { $this._overviewModal.Open() }
                    'W' { $this._store.Dispatch([ActionType]::SET_VIEW, @{ ViewName = "WeeklyReport" }) }
                    'D' {
                        $currentView = $state.View.CurrentView
                        if ($currentView -eq "Dashboard") {
                            $this._store.Dispatch([ActionType]::SET_VIEW, @{ ViewName = "Kanban" })
                        } else {
                    ...
                    Default {
                        # Some commands (like navigation arrows) don't have direct execution
                        $this._statusBar.ShowMessage("Use keyboard shortcut [$($cmd.Key)] directly", "Info")
                    }
                }
            }
            return
        ...
        if ($this._themeSelector.IsVisible()) {
            $this._themeSelector.HandleInput($key) | Out-Null
            return
        }
        ...
        # === KANBAN VIEW HANDLER ===
        if ($state.View.CurrentView -eq "Kanban") {
            [Logger]::Log("Kanban: Received key $($key.Key)", 1)
            if ($key.Key -eq 'Escape' -or $key.Key -eq 'D') {
                $this._store.Dispatch([ActionType]::SET_VIEW, @{ ViewName = "Dashboard" })
                return
            }
            ...
            switch ($key.Key) {
                'LeftArrow' {
                    if ($key.Modifiers -band [ConsoleModifiers]::Shift) {
                        $this._kanbanBoard.MoveTask("Left", $this._store, $state)
                    } else {
                        $this._kanbanBoard.MoveFocus("Left")
                    }
                }
                'RightArrow' {
                    if ($key.Modifiers -band [ConsoleModifiers]::Shift) {
                        $this._kanbanBoard.MoveTask("Right", $this._store, $state)
                    } else {
                        $this._kanbanBoard.MoveFocus("Right")
                    }
                }
                'UpArrow' { $this._kanbanBoard.MoveSelection("Up", $state) }
                'DownArrow' { $this._kanbanBoard.MoveSelection("Down", $state) }
                'Enter' {
                    $task = $this._kanbanBoard.GetSelectedTask($state)
                    if ($task) {
                        # Open Note Editor
            ...
            # Mark for re-render (Kanban state is internal class state)
            $this._forceRender = $true
            return
        }

        # === WEEKLY VIEW HANDLER ===
        [Logger]::Log("TuiApp: PAST KANBAN CHECK, about to check WeeklyReport. CurrentView=$($state.View.CurrentView)", 1)
        [Logger]::Log("TuiApp: CurrentView=$($state.View.CurrentView) checking WeeklyReport", 1)
        if ($state.View.CurrentView -eq "WeeklyReport") {
            [Logger]::Log("TuiApp: WeeklyReport matched, calling handler", 1)
            $handled = $this._weeklyView.HandleInput($key)
            if (-not $handled) {
                # Exit view on Escape (handled returns false)
                $this._store.Dispatch([ActionType]::SET_VIEW, @{ ViewName = "Dashboard" })
            }
            # Force re-render (WeeklyView state is internal class state)
            $this._forceRender = $true
            return
        }
        ...
        switch ($key.Key) {
            ...
            'T' {
                # Open Global Unified Time Modal
                $this._timeModal.Open()
            }
            'O' {
                # Open Overview modal
            ...
            'LeftArrow' {
                if ($panel -eq "Sidebar") {
                    ...
                    # Close Details panel, move focus to TaskList
                    if ($state.View.IsDetailsVisible) {
                        $this._store.Dispatch([ActionType]::TOGGLE_DETAILS, @{})
                    }
            ...
            'RightArrow' {
                 if ($panel -eq "Sidebar") {
                    ...
                    # Open Details panel (if not already visible), move focus to Details
                    if (-not $state.View.IsDetailsVisible) {
                        $this._store.Dispatch([ActionType]::TOGGLE_DETAILS, @{})
                    }
                    $this._store.Dispatch([ActionType]::SET_FOCUS, @{ PanelName = "Details" })
                } elseif ($panel -eq "Details") {
                    # Open Note Editor modal for selected task
                    $this._OpenNoteEditor($state)
                }
            }
            'UpArrow' {
            ...
            'S' {
                if ($key.Modifiers -band [ConsoleModifiers]::Control) {
                    ...
                    $fields = @(
                        ...
                        @{ Name="due"; Label="Due"; Type="Date"; Value=(Get-Date).AddDays(1).ToString("yyyy-MM-dd"); Width=12 }
                    ...
                    # Align X with Task Text Column
                     ...
                     $widths = $this._dashboard.GetColumnWidths()
                     $sidebarW = $widths.Sidebar
                     $inputX = $sidebarW + 9
            ...
            'N' {
                ...
                if ($panel -eq "Sidebar") {
                    ...
                    # Caculate Y position: Header(1) - Items start at Y+1
                    $listY = 1

                    # Robust Positioning based on Count & Panel Height
                    $projCount = if ($state.Data.projects) { $state.Data.projects.Count } else { 0 }
                    $inputY = $listY + $projCount

                    # Clamp to Panel Border (Height - 2 relative to top)
                    $pHeight = if ($widths.SidebarHeight) { $widths.SidebarHeight } else { 10 }
                    $maxY = $listY + $pHeight - 2
                    $inputY = [Math]::Min($inputY, $maxY)
                ...
                elseif ($panel -eq "TaskList") {
                    ...
                    # Align X with Task Text Column
                     ...
                     # Check count for 'Empty' handling
                     $scrollOffset = $this._dashboard.GetTaskListScrollOffset()
                     $taskCount = @($state.Data.tasks | Where-Object { $_.projectId -eq $projId }).Count

                     # FIX: If list is empty, input should mask the "Empty" text (at X+2)
                     # Standard alignment is X+9.
                     $inputX = if ($taskCount -eq 0) { $sidebarW + 2 } else { $sidebarW + 9 }

                    # Calculate inline Y position at end of current list
                    # Use Robust absolute positioning based on visual row
                    $visualRow = $taskCount - $scrollOffset

                    # If empty (count=0), visualRow=0. InputY=1.
                    # Base Y for TaskList is 1
                    $inputY = 1 + $visualRow

                    # Clamp to Panel Border (Height - 2 relative to top)
                    $tHeight = if ($widths.TaskListHeight) { $widths.TaskListHeight } else { 10 }
                    $maxY = 1 + $tHeight - 2
                    $inputY = [Math]::Min($inputY, $maxY)

                    # Also Clamp to screen to be safe
                    $inputY = [Math]::Max(1, [Math]::Min($this._engine.Height - 2, $inputY))

                    $this._smartEditor.Configure($fields, $inputX, $inputY)
                    $this._store.Dispatch([ActionType]::START_EDIT, @{ RowId="NEW"; Type="tasks"; Values=@{ projectId=$projId; status="active"; priority=1 } })
                }
                ...
                elseif ($panel -eq "Details") {
                    ...
                    $projId = $state.Data.projects[$state.View.Selection.Sidebar]['id']
                    # Inline Y for details panel
                     $widths = $this._dashboard.GetColumnWidths()
                     $detailsW = $widths.Details
                     $detailsX = $widths.Sidebar + $widths.TaskList
                     $this._smartEditor.Configure($fields, $detailsX + 2, 2)
            ...
            'E' {
                ...
                if ($panel -eq "TaskList") {
                    ...
                    if ($task) {
                        ...
                        # Calculate inline Y at selected task row
                         $widths = $this._dashboard.GetColumnWidths()
                         $sidebarW = $widths.Sidebar
                         $inputX = $sidebarW + 1
                        $scrollOffset = $this._dashboard.GetTaskListScrollOffset()
                        $visualRow = $state.View.Selection.TaskList - $scrollOffset
                        $inputY = [Math]::Max(1, [Math]::Min($this._engine.Height - 5, 1 + $visualRow))
    ...
    hidden [void] _OpenNoteEditor([hashtable]$state) {
        # Open Note Editor for selected task or project
        $desc = ""
        $title = ""
        $targetId = ""
        $type = ""
        ...
        # Get selected task
        $displayTasks = $this._GetFlattenedTasks($state)
        $task = [TaskHierarchy]::GetTaskAtIndex($displayTasks, $state.View.Selection.TaskList)

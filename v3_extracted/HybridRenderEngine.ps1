...
class HybridRenderEngine {
    ...
    [void] BeginFrame() {
        ...
        # Always fill with theme background (not dependent on retro mode)
        # This ensures black background works for Nostromo theme and other themes
        try {
    ...
    [void] DrawBox([int]$x, [int]$y, [int]$width, [int]$height, [int]$fg, [int]$bg, [string]$style) {
        ...
        # CENTRALIZED FIX: Fill the entire box area with background color first
        # This prevents ghosting/artifacts when modals close or content changes
        $this.Fill($x, $y, $width, $height, " ", $fg, $bg)
        ...
        # Draw sides only (interior already filled)
        for ($i = 1; $i -lt ($height - 1); $i++) {
            $this.WriteAt($x, $y + $i, $chars.V, $fg, $bg)
            $this.WriteAt($x + $width - 1, $y + $i, $chars.V, $fg, $bg)
        }
    ...
    [void] InvalidateCachedRegion([int]$minY, [int]$maxY) {
        ...
        # DISABLED - This was causing terminal scroll/push-up issues
        # The diff engine will handle the redraw
        # try {

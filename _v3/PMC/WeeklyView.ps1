...
class WeeklyView {
    ...
    [void] Render([HybridRenderEngine]$engine, [hashtable]$state) {
        ...
        $engine.WriteAt($startX, $headerY, "Project".PadRight($nameW), [Colors]::Accent, [Colors]::Background)
        ...
        for ($i = 0; $i -lt 7; $i++) {
            ...
            $date = $weekDates[$i]
            $header = "$d " + $date.Day
            $engine.WriteAt($x, $headerY, $header.PadLeft($colW), [Colors]::Accent, [Colors]::Background)
            $x += $colW + 1
        }
        $engine.WriteAt($x, $headerY, "Total".PadLeft($colW), [Colors]::White, [Colors]::Background)
        
        $engine.Fill($startX, $headerY + 1, $w - 4, 1, "-", [Colors]::PanelBorder, [Colors]::Background)
        ...
        foreach ($projId in $matrix.Keys) {
            ...
            if ($hasData) {
                ...
                for ($i = 0; $i -lt 7; $i++) {
                    ...
                    $col = if ($val -gt 0) { [Colors]::Success } else { [Colors]::PanelBorder }
        ...
        # Totals Row
        $engine.Fill($startX, $y, $w - 4, 1, "-", [Colors]::PanelBorder, [Colors]::Background)
        $y++
    ...
    [bool] HandleInput([ConsoleKeyInfo]$key) {
        [Logger]::Log("WeeklyView: HandleInput key=$($key.Key) weekOffset=$($this._weekOffset)", 1)
        switch ($key.Key) {
            'LeftArrow' { $this._weekOffset--; [Logger]::Log("WeeklyView: Left, new offset=$($this._weekOffset)", 1); return $true }
            'RightArrow' { $this._weekOffset++; [Logger]::Log("WeeklyView: Right, new offset=$($this._weekOffset)", 1); return $true }
